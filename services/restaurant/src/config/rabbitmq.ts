...
export const connectRabbitMQ = async () => {
  try {
    ...
    connection.on("error", (err) => {
      console.error("RabbitMQ connection error (restaurant service):", err.message);
    });

    connection.on("close", () => {
      console.error("RabbitMQ connection closed (restaurant service). Reconnecting...");
    ...
    console.error("Failed to connect to RabbitMQ (restaurant service). Retrying...", err);
    setTimeout(connectRabbitMQ, RECONNECT_DELAY_MS);
  }
};
